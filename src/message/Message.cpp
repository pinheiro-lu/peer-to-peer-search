...
std::string Message::get_message() {
    ...
    message = origin_address + ":" + std::to_string(origin_port) + " " + std::to_string(message_seqno) + " " + std::to_string(message_ttl) + " " + operation;

    if (operation == "SEARCH") {
        message += " " + mode + " " + std::to_string(last_hop_port) + " " + key + " " + std::to_string(hop_count);
    } else if (operation == "VAL") {
        message += " " + mode + " " + key + " " + value + " " + std::to_string(hop_count);
    }
    return message;
}
...
void Message::decrement_ttl() {
    ...
    // Rewrite message with new TTL
    std::ostringstream oss;
    oss << origin_address << ":" << origin_port << " " << message_seqno << " " << message_ttl << " " << operation;
